...
%% ---------------- Figure 1: Spatial ----------------
figure('Name','Spatial comparison (No SA)','Color','w');
plot(1:N, y_clean, '-k', 'LineWidth', 2); hold on;
plot(1:N, y_all(1,:), '--', 'LineWidth', 1.8);   % Iteration 1 (其實是初始 t=0)
plot(1:N, y_all(end,:), '-', 'LineWidth', 1.8);  % Final (t=Tsteps)
grid on; box on;
xlabel('Spatial index i'); ylabel('y');
title('Spatial: Ground truth vs Iter#1 vs Final (No SA)');
...
figure('Name','Feature-space trajectory (No SA)','Color','w');
plot(xgrid, ygrid, 'k-', 'LineWidth', 1.5); hold on;
grid on; box on;
xlabel('x (feature)'); ylabel('y = sin(x)');
title(sprintf('Feature space: trajectory of index %d (No SA)', idx_show));
